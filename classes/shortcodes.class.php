...
class REM_Shortcodes
{
	...
	function register_agent($attrs, $content = ''){
		if (!is_user_logged_in()) {
            ...
            wp_localize_script( 'rem-register-agent-js', 'rem_password_alert', __( 'Passwords did not match!', 'real-estate-manager' ) );
	...
	function login_agent($attrs = array(), $content = ''){
		if (is_user_logged_in()) {
	        ...
	        wp_localize_script( 'rem-login-agent', 'rem_login_wait_msg', __( 'Logging In, Please Wait...', 'real-estate-manager' ) );
    ...
    function display_maps($attrs){
	    ...
	    $args = apply_filters( 'rem_maps_porperties_args_before_render', $args );
    ...
    function single_property($attrs, $content = ''){
		if (isset($attrs['id']) && get_post_status( $attrs['id'] )) {
            ...
            // If Map is enabled
            if (rem_single_property_has_map($attrs['id'])) {
                $property_id = $attrs['id'];
			...
			return __( 'There is no property having ID ', 'real-estate-manager' ).' '.$attrs['id'];
		}
    }
		...
		<li style="padding-right: 7px;">
			...
			<?php switch ($field['type']) {
				...
				case 'select': ?>
					...
					<?php break;
				case 'image': ?>
					<input class="form-control" <?php echo $field_attrs; ?> class="img-url" type="text" value="<?php echo get_user_meta( $agent_id, $field['key'], true ); ?>" name="<?php echo $field['key']; ?>" <?php echo (isset($field['required']) && $field['required'] == 'true') ? 'required' : '' ; ?> />
		...
		<div class="<?php echo $columns; ?> space-form wrap-<?php echo $field['key']; ?>" data-condition_status="<?php echo $show_condition; ?>" data-condition_bound="<?php echo isset($field['condition_bound']) ? $field['condition_bound'] : 'all' ?>" data-condition='<?php echo json_encode($conditions); ?>'>
			...
			switch ($field['type']) {
				...
				case 'date': 
					...
					?>
					<label for="<?php echo $field['key']; ?>" class="label-p-title">
						<?php echo $field['title']; ?>
						<?php echo $help_text; ?>
