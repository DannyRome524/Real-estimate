...
function rem_get_single_property_settings_tabs(){
	$tabsData = array(
		'general_settings' => __( 'Details', 'real-estate-manager' ),
		'internal_structure' => __( 'Internal Structure', 'real-estate-manager' ),
		'property_details' => __( 'Features', 'real-estate-manager' ),
		'property_attachments' => __( 'Attachments', 'real-estate-manager' ),
		...
		case 'select2':
			...
			<select <?php echo $required; ?> multiple id="<?php echo $field['key'] ?>" name="rem_property_data[<?php echo $field['key']; ?>][]" class="form-control input-sm rem-select2-field">
				<?php
					...
					foreach ($options as $name) {
						$tr_val = (function_exists('pll_translate_string')) ? pll_translate_string(stripcslashes($name), pll_current_language()) : stripcslashes($name) ;
						$selected = (is_array($multi_value) && in_array( $tr_val,$multi_value)) ? 'selected' : '';

						echo '<option  '.$selected.' value="'.$tr_val.'" >'.stripcslashes($name).'</option>';
					}
				?>
			</select>
			<?php 
...
function rem_render_special_search_fields($key){
	...
	switch ($key) {
		case 'order': ?>
			...
			<?php break;
		case 'tags': ?>
			<select class="dropdown" data-settings='{"cutOff": 5}' name="tag" id="tag">
...
function rem_render_property_search_fields($field, $display = 'widget'){
	...
	if (array_key_exists($field['key'], $meta_keys) && rem_get_option($meta_keys[$field['key']], '') != '') { ?>
	...
	<?php } elseif ($field['type'] == 'select') { ?>
		<select class="dropdown" data-settings='{"cutOff": 5}' name="<?php echo $field['key']; ?>" id="<?php echo $field['key'].$display; ?>">
			<option value=""><?php echo $dashes; ?> <?php echo ($display == 'widget') ? __( 'Any', 'real-estate-manager' ) : $field['title']; ?> <?php echo $dashes; ?></option>
				<?php
...
function rem_get_search_query($data){
    ...
    $args = apply_filters( 'rem_search_properties_query_args', $args );
...
function rem_render_section_title($title, $tab_key){
	$wrap = apply_filters( 'rem_property_section_title_wrap', 'h3', $tab_key );
	echo "<$wrap class='title'>".stripcslashes($title)."</$wrap>";
}
...
if (is_rem_validated()) {
	require REM_PATH.'/inc/plugin-update-checker/plugin-update-checker.php';
	$myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
		'http://clients.webcodingplace.com/wcp-pro-plugins/updates/?action=get_metadata&slug=real-estate-manager-pro',
		REM_PATH.'/rem.php',
